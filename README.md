...
AI credentials are optional. Without them, Canvas, Builder, Preview, and Project Settings continue to work; page generation fails safely with “AI is not configured for this environment.” For AI operations, set `AI_PROVIDER=gemini`, a server-only `GEMINI_API_KEY`, and optionally `AI_MODEL` and `AI_PROVIDER_TIMEOUT_MS`. Run `npm run worker` beside the application. Jobs are claimed using PostgreSQL row locking, survive browser navigation, retry transient failures at most three times, and honor cancellation between stages. Gemini client-side abort stops local response handling but may not stop provider billing after submission, so cancelled responses are discarded. Run the optional paid structured-page smoke check with `npm run test:ai-provider` only when credentials are configured.
...
Builder page generation creates one immutable TSX source unit per successful job. The worker acquires the existing page editing lease, renews it during provider work, validates structured output with Zod and TypeScript AST checks, compiles it through a restricted esbuild pipeline, rechecks the submitted base version, and atomically activates the new Page Version. Generated source may import only React and `@canvas/site-runtime`; stable `CanvasImage` UUIDs resolve to temporary Preview transport URLs at runtime. Network/server APIs, arbitrary imports, dynamic code, browser storage, parent-window access, unsafe HTML, embeds, remote images, dynamic classes, and inline styles are rejected. Generated code executes only in the existing `allow-scripts` opaque-origin iframe under its restrictive CSP. The immutable version ID is the Preview compilation-cache key.
