...
- A Building Block may not embed another Building Block. Never use a data-canvas-block host inside block html.
...
- For a navbar, use c-navbar on the nav, then c-container c-cluster for its bounded layout, c-nav-brand wrapping a c-logo image, and c-nav-links containing c-link or c-button anchors. Never put c-media on an anchor.
- A block's css is scoped to that block when Canvas composes it onto a page, and its ids are made unique per usage, so name classes and ids for what they are rather than trying to avoid collisions yourself.
...
Confirm the block renders correctly standing alone on any page of this site, and that the copy names this business rather than describing a generic company. Then re-read the complete html, css, and js once against the hard contract and fix every violation, even where the construct would be valid on an ordinary website.
...
export function assembleBlockGenerationRequest(input: {
  ...
  const targetState = input.currentDocument
    ? existingDocumentPrompt("Building Block", input.currentDocument)
    : "This Building Block has no content yet. Create its first complete implementation to the design standard above.";
