...
import { aiConversations, aiMessages, mediaAssets, mediaFolders, pageNodes, projectBrandSettings, projectInstructions, projectThemeSettings } from "@/server/db/schema";
...
export type ProjectContextTarget = { type: "project" } | { type: "page"; id: string };
...
export class ProjectContextBuilder {
  ...
  async build(input: { projectId: string; actorUserId: string; target: ProjectContextTarget; selectedMediaIds?: string[]; conversationId?: string; operation?: string }) {
    ...
    const [brandRows, themeRows, instructionRows, nodeRows, folders] = await Promise.all([
    ...
    const context = {
      ...
      target: targetPage ? { id: targetPage.id, name: targetPage.name, route: targetPage.routePath, parentId: targetPage.parentId, seo: { title: targetPage.pageTitle, description: targetPage.metaDescription } } : { type: "project" as const },
    ...
    const fingerprintSource = { projectId: project.id, projectUpdatedAt: project.updatedAt.toISOString(), brandRevision: brand?.revision ?? 0, themeRevision: rawTheme?.revision ?? 0, instructionId: currentInstruction?.id ?? null, pages: nodes.map(({ id, name, routePath, updatedAt }) => ({ id, name, routePath, updatedAt: updatedAt.toISOString() })), media: assets.map(({ id, updatedAt }) => ({ id, updatedAt: updatedAt.toISOString() })), conversation: conversation.map(({ id }) => id), target: context.target };
    return { ...context, fingerprint: createHash("sha256").update(stable(fingerprintSource)).digest("hex"), composition: { pageCount: context.structure.pages.length, mediaCount: context.media.length, conversationMessageCount: conversation.length, instructionRevision: context.instructions.revisionNumber } };
  }
}
