...
import { Check, CircleAlert, ExternalLink, FileText, Folder, House, LoaderCircle, Maximize2, Monitor, Moon, RefreshCw, Send, Sparkles, Smartphone, Sun, Tablet, X } from "lucide-react";
...
type BuilderAIState = { page: { id: string; currentVersionId: string | null }; conversation: { id: string } | null; messages: Array<{ id: string; role: "user" | "assistant" | "system_internal"; content: string; createdAt: string }>; job: null | { id: string; status: string; progressStage: string; errorMessage: string | null; resultPageVersionId: string | null } };
...
export function BuilderWorkspace({ projectId, initialSession, initialPageId, initialInstanceId, mediaAssets, mediaFolders }: { projectId: string; initialSession: PreviewSession; initialPageId?: string; initialInstanceId: string; mediaAssets: MediaAsset[]; mediaFolders: MediaFolder[] }) {
  ...
  const loadAIState = useCallback(async (pageId: string) => {
    const response = await fetch(`/api/projects/${projectId}/pages/${pageId}/ai`, { cache: "no-store" }); const value = await response.json() as BuilderAIState & { error?: string };
    if (!response.ok) throw new Error(value.error || "Canvas history could not be loaded."); setAIState(value); return value;
  }, [projectId]);
  useEffect(() => { if (!currentPageId) return; let active = true; const timer = window.setTimeout(() => { if (active) { setAIState(null); setAIError(undefined); setPrompt(""); setSelectedMediaIds([]); setAILoading(true); } void loadAIState(currentPageId).catch((cause) => { if (active) setAIError(cause instanceof Error ? cause.message : "Canvas history could not be loaded."); }).finally(() => { if (active) setAILoading(false); }); }, 0); return () => { active = false; window.clearTimeout(timer); }; }, [currentPageId, loadAIState]);
  const activeJob = aiState?.job && ACTIVE_JOB_STATUSES.has(aiState.job.status) ? aiState.job : null;
  useEffect(() => { if (!currentPageId || !activeJob) return; const timer = window.setInterval(() => { void loadAIState(currentPageId).catch(() => undefined); }, 1_500); return () => window.clearInterval(timer); }, [activeJob, currentPageId, loadAIState]);
  ...
  async function submitPrompt() {
    if (!currentPageId || !prompt.trim() || activeJob) return; setAILoading(true); setAIError(undefined);
    try { const response = await fetch(`/api/projects/${projectId}/pages/${currentPageId}/ai`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ content: prompt, selectedMediaIds }) }); const value = await response.json() as { error?: string }; if (!response.ok) throw new Error(value.error || "Canvas could not start this update."); await loadAIState(currentPageId); }
  ...
  async function cancelJob() { if (!activeJob) return; setAILoading(true); try { const response = await fetch(`/api/projects/${projectId}/generation-jobs/${activeJob.id}`, { method: "DELETE" }); const value = await response.json() as { error?: string }; if (!response.ok) throw new Error(value.error || "This update could not be cancelled."); if (currentPageId) await loadAIState(currentPageId); } catch (cause) { setAIError(cause instanceof Error ? cause.message : "This update could not be cancelled."); } finally { setAILoading(false); } }
...
function BuilderAIComposer({ page, state, loading, error, prompt, selectedMediaIds, assets, folders, activeJob, onPrompt, onMedia, onSubmit, onCancel }: { page: ProjectPreviewManifest["pages"][number] | null; state: BuilderAIState | null; loading: boolean; error?: string; prompt: string; selectedMediaIds: string[]; assets: MediaAsset[]; folders: MediaFolder[]; activeJob: BuilderAIState["job"]; onPrompt: (value: string) => void; onMedia: (ids: string[]) => void; onSubmit: () => void; onCancel: () => void }) {
  const latestSummary = [...(state?.messages ?? [])].reverse().find((message) => message.role === "assistant");
  return <section className="builder-ai" aria-label="Canvas AI"><div className="builder-ai-title"><Sparkles size={15} /><strong>Canvas</strong></div><div className="builder-ai-history">{state?.messages.slice(-6).map((message) => <div key={message.id} className={`builder-ai-message ${message.role === "user" ? "from-user" : "from-canvas"}`}><small>{message.role === "user" ? "You" : "Canvas"}</small><p>{message.content}</p></div>)}{loading && !state ? <p className="inline-empty"><LoaderCircle className="spin" size={13} /> Loading history…</p> : null}</div>{latestSummary && state?.job?.status === "completed" ? <div className="builder-ai-summary"><strong>Canvas updated this page</strong><p>{latestSummary.content}</p></div> : null}{activeJob ? <div className="builder-ai-progress" role="status" aria-live="polite"><span><LoaderCircle className="spin" size={14} />{activeJob.progressStage}</span><Button type="button" variant="ghost" onClick={onCancel} disabled={loading}>Cancel</Button></div> : null}{state?.job?.status === "failed" ? <p className="builder-ai-error" role="alert"><CircleAlert size={14} />{state.job.errorMessage || "Canvas could not update this page. Try again."}</p> : null}{error ? <p className="builder-ai-error" role="alert"><CircleAlert size={14} />{error}</p> : null}<label className="field"><span className="field-label">{page?.contentStatus === "generated" ? "Ask Canvas to change this page" : "Describe the page you want Canvas to create"}</span><textarea className="textarea builder-ai-textarea" value={prompt} maxLength={AI_LIMITS.userMessageCharacters} rows={4} disabled={!page || Boolean(activeJob)} placeholder={page?.contentStatus === "generated" ? "Make the hero shorter and improve the mobile spacing…" : "Create a modern homepage with a hero, services, and contact call-to-action…"} onChange={(event) => onPrompt(event.target.value)} /><span className="field-hint">{prompt.length.toLocaleString()} / {AI_LIMITS.userMessageCharacters.toLocaleString()}</span></label><div className="builder-ai-attachments">{selectedMediaIds.map((id) => { const asset = assets.find((item) => item.id === id); return asset ? <span key={id}><Image src={`/api/media/${id}`} width={24} height={24} alt="" unoptimized /><span>{asset.displayName}</span><button type="button" aria-label={`Remove ${asset.displayName}`} onClick={() => onMedia(selectedMediaIds.filter((item) => item !== id))}><X size={12} /></button></span> : null; })}</div><div className="builder-ai-actions"><MultiMediaPicker assets={assets} folders={folders} value={selectedMediaIds} limit={PAGE_MEDIA_ATTACHMENT_LIMIT} onSelect={onMedia} /><Button type="button" onClick={onSubmit} disabled={!page || !prompt.trim() || Boolean(activeJob) || loading}><Send size={14} />{page?.contentStatus === "generated" ? "Update page" : "Create page"}</Button></div></section>;
}
