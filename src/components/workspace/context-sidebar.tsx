...
export function ContextSidebar({ projectId, activity, mediaAssets, mediaFolders, blocks, website, history, historySection, onOpenPanel, onNewBlock, onAddSection, canAddSection, onHistorySection }: { projectId: string; activity: WorkspaceActivity; mediaAssets: MediaAsset[]; mediaFolders: MediaFolder[]; blocks: ProjectPreviewManifest["blocks"]; website: ReactNode; history: HistoryController; historySection: HistorySection; onOpenPanel: (name: string, query?: Record<string, string>) => void; onNewBlock: () => void; onAddSection: () => void; canAddSection: boolean; onHistorySection: (section: HistorySection) => void }) {
  ...
  // Adding a section to the page you are looking at is the common act, so it leads.
  // "New" still exists for building one from scratch with the agent.
  if (activity === "sections") { const values = Object.values(blocks); return <SidebarShell title="Reusable Sections" action={<><button type="button" className="ws-side-action ws-side-action-icon" aria-label="Add a section to this page" disabled={!canAddSection} title={canAddSection ? "Add a section to this page" : "Open a page first"} onClick={onAddSection}><Plus size={15} /></button><button type="button" className="ws-side-action ws-side-action-icon" aria-label="Create a reusable section" title="Create a reusable section" onClick={onNewBlock}><Blocks size={15} /></button></>} footer={count(values.length, "reusable section")}>
    {!values.length ? <SidebarEmpty icon={<Blocks size={20} />} title="No reusable sections" text="Add a ready-made navbar, footer or hero from the Canvas library, or build one with the agent." action="Add a section" onAction={onAddSection} /> : <div className="ws-side-list">{values.map((block) => <button type="button" key={block.id} onClick={() => onOpenPanel("blocks", { block: block.id })}><Blocks size={15} /><span><strong>{block.name}</strong><small>{block.isGlobal ? "Used across the website" : block.contentStatus === "generated" ? "Ready to use" : "Draft"}</small></span><ChevronRight size={14} /></button>)}</div>}
  </SidebarShell>; }
