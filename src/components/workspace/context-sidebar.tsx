...
import { Blocks, ChevronRight, History, ImageIcon, Palette, Plus, Settings2 } from "lucide-react";
...
export function ContextSidebar({ activity, mediaAssets, mediaFolders, blocks, website, onOpenPanel, onNewBlock, onHistory }: { activity: WorkspaceActivity; mediaAssets: MediaAsset[]; mediaFolders: MediaFolder[]; blocks: ProjectPreviewManifest["blocks"]; website: ReactNode; onOpenPanel: (name: string) => void; onNewBlock: () => void; onHistory: () => void }) {
  ...
  return <SidebarShell title="History" action={<button type="button" className="ws-side-action" onClick={onHistory}>Open</button>} footer="Immutable versions and checkpoints"><div className="ws-side-sections"><SidebarLink icon={<History size={17} />} title="Recent changes" text="Review, undo, and restore committed work" onClick={onHistory} /><SidebarLink icon={<History size={17} />} title="Checkpoints" text="Save or restore the whole website" onClick={onHistory} /></div></SidebarShell>;
}
