...
export function ThemeEditor({ projectId, initialBrand, initialTheme, recoveredFromInvalidState = false }: {
  ...
  return <div className="theme-editor-layout"><div className="theme-settings-column">
    ...
    <Card><div className="settings-title"><div><p className="eyebrow">Theme</p><h2>Colors</h2></div><SaveIndicator status={themeStatus} error={themeError} /></div><div className="segmented" role="group" aria-label="Theme color mode"><button type="button" className={mode === "light" ? "active" : ""} onClick={() => setMode("light")}><Sun size={14} />Light</button><button type="button" className={mode === "dark" ? "active" : ""} onClick={() => setMode("dark")}><Moon size={14} />Dark</button></div>{themeError ? <p className="form-error" role="alert">{themeError}</p> : null}<div className="color-grid">{COLOR_FIELDS.map((field) => <ColorField key={field.key} label={field.label} value={theme[mode === "light" ? "lightTokens" : "darkTokens"][field.key]} onChange={(value) => updateColor(field.key, value)} />)}</div></Card>
    <Card><div className="settings-title"><div><p className="eyebrow">System</p><h2>Style</h2></div></div><div className="scale-list">{SCALE_FIELDS.map((field) => <ScaleField key={field.key} label={field.label} low={field.low} high={field.high} value={theme[field.key]} onChange={(value) => updateTheme({ [field.key]: value })} />)}</div><div className="reset-row"><div><strong>Restore visual defaults</strong><p>Resets both color modes and all style controls. Brand details stay unchanged.</p></div><ConfirmationDialog title="Reset theme?" triggerLabel="Reset theme" description="Both light and dark colors and all style controls will return to their defaults. Company identity will be preserved." action={<Button type="button" variant="danger" onClick={() => void resetTheme()}><RotateCcw size={15} />Reset theme</Button>} /></div></Card>
  </div>
  <aside className="theme-preview-column"><div className="preview-heading"><div><p className="eyebrow">Live preview</p><h2>{brand.companyName || "Your company"}</h2></div><div className="segmented compact" role="group" aria-label="Preview mode"><button type="button" className={mode === "light" ? "active" : ""} onClick={() => setMode("light")}>Light</button><button type="button" className={mode === "dark" ? "active" : ""} onClick={() => setMode("dark")}>Dark</button></div></div><div className="project-theme-preview" style={previewStyle}><nav><strong>{brand.companyName || "Company"}</strong><span>About&nbsp;&nbsp; Work&nbsp;&nbsp; Contact</span></nav><main><span className="preview-badge">New perspective</span><h3>Build something great</h3><p>{brand.companyDescription || "A sample heading and supporting body copy that demonstrates your project’s typography and colors."}</p><div className="preview-buttons"><button>Primary button</button><button>Secondary button</button></div><section><h4>Example card</h4><p>This surface demonstrates your spacing, borders, radius, shadows, and muted text.</p><label>Example input<input placeholder="Type something…" /></label><a href="#preview">Text link</a></section></main></div></aside>
  </div>;
}
