...
export function AcceptInviteForm({ token, alreadyHasAccess }: { token: string; alreadyHasAccess: boolean }) {
  ...
  return <form action={action} className="stack"><input type="hidden" name="token" value={token} />{state.error ? <p className="form-error" role="alert">{state.error}</p> : null}<SubmitButton pendingLabel="Joining…">{alreadyHasAccess ? "Open website" : "Join website"}</SubmitButton></form>;
}
