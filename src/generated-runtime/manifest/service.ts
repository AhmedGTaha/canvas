...
export class PreviewManifestService {
  ...
  private async build(userId: string, projectId: string, sessionId: string, token: string) {
    ...
    const relevantState = JSON.stringify({ pages: pages.map((page) => [page.pageId, page.canonicalRoute, page.name, page.currentVersionId, page.seo]), brand: [brand.revision, brand.primaryLogoMediaId, brand.alternateLogoMediaId], theme: theme.revision, media: library.assets.map((asset) => [asset.id, asset.updatedAt.toISOString()]) });
