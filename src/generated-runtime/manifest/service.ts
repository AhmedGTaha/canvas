...
export class PreviewManifestService {
  ...
  private async build(userId: string, projectId: string, sessionId: string, token: string) {
    const [nodes, brand, theme, library, blockList] = await Promise.all([new PageTreeService().listTree(userId, projectId), new BrandService().read(userId, projectId), new ThemeService().read(userId, projectId), new MediaService().list(userId, { projectId }), new BuildingBlockService().listForContext(projectId)]);
    const blocks = Object.fromEntries(blockList.map((block) => [block.id, { id: block.id, name: block.name, kind: block.kind, isGlobal: block.isGlobal, activeVersionId: block.currentVersionId, contentStatus: block.currentVersionId ? "generated" as const : "unbuilt" as const }]));
    const pages = nodes.filter((node) => node.type === "page" && node.routePath).map((node) => ({ pageId: node.id, parentId: node.parentId, name: node.name, canonicalRoute: node.routePath!, isHomepage: node.isHomepage, currentVersionId: node.currentVersionId, contentStatus: node.currentVersionId ? "generated" as const : "unbuilt" as const, seo: { title: node.pageTitle, description: node.metaDescription } }));
    ...
    const relevantState = JSON.stringify({ pages: pages.map((page) => [page.pageId, page.canonicalRoute, page.name, page.currentVersionId, page.seo]), brand: [brand.revision, brand.primaryLogoMediaId, brand.alternateLogoMediaId], theme: theme.revision, media: library.assets.map((asset) => [asset.id, asset.updatedAt.toISOString()]), blocks: blockList.map((block) => [block.id, block.currentVersionId, block.isGlobal, block.name]) });
    return projectPreviewManifestSchema.parse({ manifestVersion: 1, projectId, previewSessionId: sessionId, generatedAt: new Date().toISOString(), previewRevision: createHash("sha256").update(relevantState).digest("hex").slice(0, 20), homepage: pages.find((page) => page.isHomepage)?.pageId ?? null, routes, pages, brand: { companyName: brand.companyName, companyDescription: brand.companyDescription, primaryLogoMediaId, alternateLogoMediaId, logoMediaIds: { light: primaryLogoMediaId, dark: alternateLogoMediaId ?? primaryLogoMediaId } }, theme: theme.resolvedDesignTokens, media, blocks, navigation: byParent(null) });
  }
}
