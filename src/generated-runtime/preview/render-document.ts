...
function textEditor(target,region){let node=target&&target.nodeType===1?target:null;while(node&&node!==region){if(node.childNodes.length===1&&node.firstChild&&node.firstChild.nodeType===3)return node;node=node.parentElement}return null}
function textPosition(region,editor){const walker=document.createTreeWalker(region,NodeFilter.SHOW_TEXT);let current,index=0;while((current=walker.nextNode())){if(current===editor.firstChild)return index;index++}return-1}
document.addEventListener("dblclick",event=>{if(!selection.mode)return;const region=selectable(event.target),editor=region&&textEditor(event.target,region);if(!region||!editor)return;event.preventDefault();event.stopPropagation();selectNode(region,true);const original=editor.textContent||"";editor.contentEditable="true";editor.focus();const range=document.createRange();range.selectNodeContents(editor);const pick=getSelection();pick&&pick.removeAllRanges();pick&&pick.addRange(range);const finish=()=>{editor.removeEventListener("blur",finish);editor.contentEditable="false";const text=editor.textContent||"",index=textPosition(region,editor);if(index>=0&&text!==original){const owner=ownerOf(region);send({type:"CANVAS_TEXT_UPDATED",canvasId:region.getAttribute("data-canvas-id"),blockId:owner.blockId,textIndex:index,text})}};editor.addEventListener("blur",finish,{once:true})},true);
