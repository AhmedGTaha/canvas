...
export default async function ProjectPage({ params }: { params: Promise<{ projectId: string }> }) {
  ...
  return <><PageHeader eyebrow="Project overview" title={project.name} description={project.description || "Add a description as this project takes shape."} actions={<><Link href={`/projects/${project.id}/collaborators`} className={buttonClass("secondary")}><UsersRound size={16} />Collaborators</Link>{role === "owner" ? <RenameProjectDialog id={project.id} name={project.name} /> : null}</>} />
