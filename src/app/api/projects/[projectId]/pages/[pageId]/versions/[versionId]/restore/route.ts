...
export async function POST(_request: Request, { params }: { params: Promise<{ projectId: string; pageId: string; versionId: string }> }) {
  const user = await getCurrentUser();
  if (!user) return Response.json({ error: "Authentication required." }, { status: 401, headers: { "Cache-Control": "no-store" } });
  try {
    const { projectId, pageId, versionId } = await params;
    const result = await new VersionRestoreService().restorePageVersion(user.id, projectId, pageId, versionId);
